...
describe('Torrents', () => {
  ...
  describe('Download', () => {
    ...
    it('Regular request succeed', () => {
      return wrapped.run({pathParameters: {productId: initialTorrent.productId, torrentId: initialTorrent.torrentId}}).then(response => expect(response.statusCode).to.be.equal(200))
    })
